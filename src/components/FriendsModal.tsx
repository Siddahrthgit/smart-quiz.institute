...
export const FriendsModal: React.FC<FriendsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-slate-50 border border-slate-200 w-full max-w-3xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        {/* Modal Header */}
        ...
        <div className="p-5 border-b border-slate-200 space-y-4 bg-slate-50/50">
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-800 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all"
            />
          </div>
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                activeTab === 'my-friends'
                  ...
                  : 'bg-slate-100/80 text-slate-600 hover:text-slate-800'
              }`}
            >
        ...
        <div className="p-5 overflow-y-auto space-y-3 flex-1">
          {isLoading ? (
            <div className="py-12 text-center space-y-3">
              ...
              <p className="text-xs text-slate-600">Searching network...</p>
            </div>
          ) : (activeTab === 'my-friends' ? myFriends : suggestedUsers).length === 0 ? (
            <div className="py-12 text-center space-y-2">
              <Users className="w-10 h-10 text-slate-400 mx-auto" />
              <p className="text-xs font-semibold text-slate-700">No friends found matching your search</p>
            ...
            (activeTab === 'my-friends' ? myFriends : suggestedUsers).map((user) => {
              ...
              return (
                <div
                  key={user.id}
                  className="bg-white border border-slate-200 hover:border-slate-300/80 p-4 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all"
                >
                  <div className="flex items-center space-x-3">
                    <div className="relative">
                      <img
                    ...
                    <div className="space-y-0.5">
                      ...
                      {user.lastExamTitle && (
                        <div className="flex items-center space-x-2 text-[10px] text-slate-600 pt-1">
                          <span className="text-slate-500">Latest Exam:</span>
                          <span className="text-slate-700 font-medium">{user.lastExamTitle}</span>
                  ...
                  <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-4 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-200">
                    <div className="flex items-center space-x-3 text-center">
                      <div className="bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                        <span className="text-[9px] text-slate-600 block font-semibold uppercase">Exam Gain</span>
                      ...
                      <div className="bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                        <span className="text-[9px] text-slate-600 block font-semibold uppercase">Streak</span>
                      ...
                      <div className="bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200">
                        <span className="text-[9px] text-slate-600 block font-semibold uppercase">XP</span>
